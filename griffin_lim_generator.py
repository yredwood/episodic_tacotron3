...
# ========== parameters ===========
checkpoint_path = 'models/lin_spec_re/checkpoint_22000'
...
def linear_decode(lin_pred):
    ...
    mel_decompress = taco_stft.spectral_de_normalize(lin_pred)
...
def waveglow_decode(mel_pred):
    waveform = denoiser(waveglow.infer(mel_pred, sigma=0.8), 0.01)[:,0]
    return waveform

def decode(lin_pred, mel_pred):
    if use_griffin_lim:
        return linear_decode(lin_pred)
    else:
        return waveglow_decode(mel_pred)
...
if hparams.model_name == 'episodic-transformer':
    ...
    for i in range(num_support_save):
        ...
        audio = decode(lin_spec[:,:,:lin_len], mel_spec[:,:,:lin_len])
        ...
        with torch.no_grad():
            ...
            audio = decode(outputs['lin_post'], outputs['mel_post'])
